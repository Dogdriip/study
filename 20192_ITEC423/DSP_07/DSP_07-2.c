...
tNodeptr merge(tNodeptr bt1, tNodeptr bt2) {  // 두 Binomial tree merge
    if (bt1->data < bt2->data) {  // 일관성을 위해 1번 트리가 2번 트리보다 항상 크게
        tNodeptr tmp = bt1;
        bt1 = bt2;
        bt2 = tmp;
    }
...
tNodeptr get_maxval(nodeptr head) {  // Heap 루트 원소들 중 최댓값
...
nodeptr pop(nodeptr head) {  // heap에서의 pop
    ...
    nodeptr curr = head->rlink;
    nodeptr tmp_node;
    tNodeptr tmp = get_maxval(head);  // 최댓값 찾아오기

    // 최댓값 노드를 제외한 노드들로 다시 heap 구성
    while (curr != head) {
        if (curr->data != tmp) {
            tmp_node = (nodeptr)malloc(sizeof(*tmp_node));
...
int main(int argc, char* argv[]) {
    ...
    fprintf(fp2, "After deleting two max values:\n");
    bt1 = pop(bt1);
    bt1 = pop(bt1);
    print_heap(bt1);
    fprintf(fp2, "\n");
